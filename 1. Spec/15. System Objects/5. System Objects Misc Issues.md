...
The\* __Referrers__ list consists of references *back* to the\* referrers, but\* that does *not\__ mean the\* object in turn becomes a referrer of the\* referrer again.
...
But\* then\* the\* __Number__ class will also register all its *class referrers*, which is undoable, because\* a humungous amount of objects refer to this class. But\* a solution for this was already proposed by the\* article *Class Referrers*. You\* can\* choose for a class to not\* register its class referrers, while objects do register their referrers.
